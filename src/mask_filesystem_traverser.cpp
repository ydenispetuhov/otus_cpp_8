...
bool MaskFilesystemTraverser::check_file(const fs::path& file) const
{
    ...
    const auto& filename_cstr = file.filename().string();

    for (const std::regex& re : m_file_masks_exclude)
        if (std::regex_match(filename_cstr, re))
            return false;

    if (m_file_masks_include.empty())
        return true;

    return std::any_of(m_file_masks_include.begin(), m_file_masks_include.end(), [&](const std::regex& re) {
        return std::regex_match(filename_cstr, re);
    });
}
